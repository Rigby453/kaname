...
- [ ] **БАГ: недельная вода показывает СУММУ, а не среднее/день.** За неделю записано 4750 мл (тест) —
  в недельном виде показывается 4750, а должно быть среднее за день = всего/7 (≈679 мл/день). Файлы:
  `health/water_report_screen.dart` / `health_screen.dart` (weekWaterProvider). Для недельного периода
  показывать avg/день (сумма/число дней), подписать «в среднем/день». Сверить так же сон/выполнение —
  чтобы недельные сводки везде были средними, а не суммами. Войдёт во флаттер-батч.
