...
- [x] **Иностранный OAuth (Google/Apple) — НЕ добавляем** (решение, консервативное прочтение 406-ФЗ):
  «доп-кнопка при наличии РФ-метода» — спорная зона при реальных штрафах; владелец РФ → исключаем
  иностранный OAuth полностью. (Anthropic/Claude может — иностранная компания, закон на неё не
  распространяется; нам копировать нельзя.) Итог метода входа: email(любой)+пароль и телефон+пароль,
  всё на своём бэкенде. NB: финал сверить с юристом под орг-форму (особенно ИП).
