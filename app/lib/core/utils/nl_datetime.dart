...
///
/// ИСТОЧНИК ИСТИНЫ: module_inference.dart::kModuleInferenceKeywords.
/// Эта переменная — производная от него: конвертируем [ModuleInferenceKey] →
/// [_Keyword], чтобы парсер использовал ОДИН словарь, а не независимую копию.
/// При добавлении/изменении ключевых слов правьте ТОЛЬКО module_inference.dart.
final List<(List<_Keyword>, String)> _moduleKeywords =
    kModuleInferenceKeywords
        .map(
          (entry) => (
            entry.$1
                .map((k) => _Keyword(k.text, wholeWord: k.wholeWord))
                .toList(),
            entry.$2,
          ),
        )
