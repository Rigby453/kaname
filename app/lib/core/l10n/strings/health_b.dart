...
const Map<String, Map<String, String>> healthBStrings = {
  ...
  // Day/Week/Month переключатель (#22) — заголовок графика (столбцы по дням +
  // линия тренда). EN/RU; остальные языки откатываются на EN.
  // Пустое состояние истории (sleep.empty_hint/sleep.log_sleep) уже звучит
  // нейтрально к периоду — отдельный ключ для Недели/Месяца не нужен.
  'sleep.chart_section': {
    'en': 'Trend',
    'ru': 'Тренд',
  },
  ...
  // ---------------------------------------------------------------------------
  // water.period_* — переключатель Day/Week/Month отчёта воды (#22) + фикс
  // недельного headline'а (сумма → среднее/день, триаж water-weekly-headline-sum).
  // EN/RU; остальные языки откатываются на EN.
  // ---------------------------------------------------------------------------

  // Главный показатель для недели/месяца — среднее в день (а не сумма за
  // период, см. триаж: было 4750 вместо 679/день).
  'water.stat_avg_day': {
    'en': 'Avg / day',
    'ru': 'В среднем/день',
  },
  // Кол-во дней периода, где норма была выполнена, напр. «4/7».
  'water.stat_days_met': {
    'en': 'Goal days',
    'ru': 'Дней с целью',
  },
  // Заголовок секции графика (столбцы + линия тренда) над списком по дням.
  'water.chart_section': {
    'en': 'Trend',
    'ru': 'Тренд',
  },
  // Заголовок секции суточных итогов в режиме Неделя/Месяц (вместо
  // поминутного списка записей, который показывается только в режиме Day).
  'water.daily_totals_section': {
    'en': 'By day',
    'ru': 'По дням',
  },
  // Пустое состояние для Недели/Месяца без единой записи воды.
  'water.no_data_period': {
    'en': 'No water logged in this period',
    'ru': 'За этот период нет записей о воде',
  },
